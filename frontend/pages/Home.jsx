...
const Home = () => {
  ...
  return (
    <div className='h-screen bg-[#F2FAFA]'>
      ...
      <main className=' m-5 flex justify-center items-center gap-[40px]'>
        <motion.div initial={{ opacity: 0, x: 0 }} animate={{ opacity: 1, x: 30 }}
          transition={{ duration: 0.8, ease: "easeIn" }}
          className='w-[500px] h-[500px] mt-10'> <h1 className='text-4xl font-bold text-black '>Are you finding local clients?</h1>
          <p className='text-xl font-semibold w-[500px] mt-5 text-center text-[#252525]'>Find work near you. Fast.
            Browse local gigs, connect with nearby clients, and get paid for what you do best
            — all in your area.</p>
        <div className='flex items-center justify-center mt-10'>
          <button className='bg-sky-500 text-white font-semibold px-10 py-4
             rounded-md  cursor-pointer
            ' onClick={()=>handleNearbyProjects()}>Check for Projects</button>
          <input type="search" name="job-search" id="job-search"
            placeholder='Search freelance work' className=' ml-5 px-4 py-4 border-2 border-sky-500 rounded-md outline-none
             transition-all duration-200  focus:border-sky-500 focus:ring-2 focus:ring-sky-200
             '  />
          <span className=" items-center ml-3">
